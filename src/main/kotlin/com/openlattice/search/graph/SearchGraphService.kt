...
class SearchGraphService(
    ...
    fun executeEntityNeighborSearch(
        ...
        graphService.getEdgesAndNeighborsForVerticesBulk(
                ...
                .forEach { edge ->
                    ...
                    allEntitySetIds.add(if (entityKeyIds.contains(edge.src.entityKeyId))
        ...
        val authorizedEntitySetIds = getReadAuthorizedIds(allEntitySetIds, principals, true)

        val entitySetsById = entitySetService.getEntitySetsAsMap(authorizedEntitySetIds)

        val entitySetsIdsToAuthorizedProps = Maps.newHashMapWithExpectedSize<UUID, MutableMap<UUID, PropertyType>>(entitySetsById.size)
        val authorizedEdgeESIdsToVertexESIds = Maps.newHashMapWithExpectedSize<UUID, MutableSet<UUID>>(entitySetsById.size)
        val entityTypesById = dataModelService
                .getEntityTypesAsMap(entitySetsById.values
                        .map { entitySet ->
        ...
        val propertyTypesById = dataModelService
                .getPropertyTypesAsMap(entityTypesById.values.flatMap { entityType -> entityType.properties }.toSet())
        ...
        authorizations.accessChecksForPrincipals(accessChecks, principals).forEach { auth ->
            if (auth.permissions.getValue(Permission.READ)) {
                ...
                entitySetsIdsToAuthorizedProps.getValue(esId)[propertyTypeId] = propertyTypesById.getValue(propertyTypeId)
            }
        }
        logger.info(
        ...
        val entitiesAcrossEntitySetIds = dataManager
    ...
    private fun executeLinkingEntityNeighborSearch(
        ...
        val linkingEntityNeighbors = Maps.newConcurrentMap<UUID, MutableList<NeighborEntityDetails>>()
        ...
        filter.entityKeyIds.forEach { (linkedEntitySetId, linkingIds) ->
            ...
            graphService
                    .getEdgesAndNeighborsForVerticesBulk(
                            EntityNeighborsFilter(
                                    normalEntitySetIds.map { it to entityKeyIds }.toMap(),
                                    filter.srcEntitySetIds,
                                    filter.dstEntitySetIds,
                                    filter.associationEntitySetIds))
                    .forEach { edge ->
            ...
            val entitySetsIdsToAuthorizedProps =
                    Maps.newHashMapWithExpectedSize<UUID, MutableMap<UUID, PropertyType>>(entitySetsById.size)
            val authorizedEdgeESIdsToVertexESIds =
                    Maps.newHashMapWithExpectedSize<UUID, MutableSet<UUID>>(entitySetsById.size)
            ...
            val accessChecks = entitySetsById.values
                    .flatMap { entitySet ->
                        entityTypesById.getValue(entitySet.entityTypeId).properties.map { propertyTypeId ->
                            AccessCheck(AclKey(entitySet.id, propertyTypeId), EdmAuthorizationHelper.READ_PERMISSION)
                        }
                    }.toSet()
            ...
            edges.parallelStream().forEach { edge ->
                ...
                val neighbor = getNeighborEntityDetails(edge,
                        authorizedEdgeESIdsToVertexESIds,
                        entitySetsById,
                        vertexIsSrc,
                        entities)
            ...
            entityKeyIdsByLinkingId.forEach { (linkingId, normalEntityKeyIds) ->
                linkingEntityNeighbors[encryptedLinkingIds[linkingId]] = normalEntityKeyIds
    ...
    fun executeEntityNeighborIdsSearch(
        ...
        val unauthorizedEntitySetIds = getReadAuthorizedIds(allEntitySetIds, principals, false)
    ...
    private fun getReadAuthorizedIds(
